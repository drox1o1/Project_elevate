...
import type { Motif } from "@/lib/pop-indices/types";
...
function Parcel({ v, sw }: { v: number[]; sw: number }) {
  // A plot subdividing: each observation carves the boundary a little finer,
  // which is what rising land value does to a family holding.
  const cuts = resample(v, 9);
  return (
    <>
      <rect x={18} y={30} width={164} height={140} strokeWidth={sw * 1.6} />
      {cuts.map((n, i) => {
        const x = 18 + ((i + 1) / (cuts.length + 1)) * 164;
        return (
          <line
            key={`v${i}`}
            x1={x}
            x2={x}
            y1={30}
            y2={30 + 140 * (0.25 + n * 0.75)}
            strokeWidth={sw}
            opacity={0.3 + n * 0.6}
          />
        );
      })}
      <line x1={18} x2={182} y1={30 + 140 * 0.62} y2={30 + 140 * 0.62} strokeWidth={sw} opacity={0.5} />
    </>
  );
}
