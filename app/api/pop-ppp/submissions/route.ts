...
/**
 * Reference submissions for Pop PPP.
 *
 ...
 *
 * Storage: forwarded to DUKU_POP_PPP_WEBHOOK when set, logged server-side
...
export async function POST(req: NextRequest) {
  ...
  const submission = {
    ...
    source: "labs.duku.design/pop-ppp",
  };

  const webhook = process.env.DUKU_POP_PPP_WEBHOOK;
  if (webhook) {
    try {
      await fetch(webhook, {
        method: "POST",
      ...
      console.error("[pop-ppp] webhook forward failed:", err);
    }
  }
  console.log("[pop-ppp]", JSON.stringify(submission));
