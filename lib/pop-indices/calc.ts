/**
 * Pop Indices calculation framework (PRD §11).
 *
...
export function valueAt(series: Series, year: number): number {
  ...
  if (!obs) {
    throw new Error(
      `Pop Indices: no observation for ${year} in series "${series.id}".`
    );
  }
  return obs.value;
}
 ...
 */
export type IncomeUnit = "years" | "months" | "days" | "hours" | "minutes";
...
export function incomeUnitFor(maxMonths: number): IncomeUnit {
  ...
  // A Delhi plot costs thousands of months of average income. Past three
  // years the figure stops being readable as months.
  if (maxMonths >= 36) return "years";
 ...
 *
 * Locale matters here: en-IN groups by lakh, so a dollar index's +101,900%
 * renders as "+1,01,900%" unless the caller passes its own locale.
 */
export function formatPercent(value: number, locale = "en-IN"): string {
  if (!Number.isFinite(value)) return "—";
  const abs = Math.abs(value);
  const decimals = abs >= 1000 ? 0 : 1;
  return `${value >= 0 ? "+" : "−"}${abs.toLocaleString(locale, {
    minimumFractionDigits: decimals,
    maximumFractionDigits: decimals,
  })}%`;
}

export function formatMultiple(value: number, locale = "en-IN"): string {
  if (!Number.isFinite(value)) return "—";
